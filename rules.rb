...
Maid.rules do
  ...
  rule 'Misc Screenshots' do
    dir('~/Desktop/Screen shot *').each do |path|
      if 1.week.since?(accessed_at(path))
        move(path, '~/Desktop/PileStuff/Old_Screenshots/')
      end
    end
  end
  ...
  rule 'downloaded PDF' do
    move(dir('~/Downloads/*.pdf'),'~/Downloads/pdf/')
  end

  rule 'airdrop photo' do
    dir('~/Downloads/IMG_*').each do |path|
      if 1.week.since?(accessed_at(path))
            move(path, '~/Downloads/airdrop/')
      end
    end
  end

  rule 'downloaded documents' do
    dir('~/Downloads/*.{doc,docx,ppt,pptx,xls,xlsx}').each do |path|
      if 1.week.since?(accessed_at(path))
            move(path, '~/Downloads/downloadedDocuments/')
      end
    end
  end
